declare const SKYChatExtension: any, SKYChatCacheController: any, SKYRecord: any, SKYConversation: any, SKYMessage: any;
...
export class Chat {
    ...
    async fetchConversation(conversationId: string) {
        try {
            let worker = await this.spawnWorker();
            await this.chat
                .fetchConversationWithConversationIDFetchLastMessageCompletion(
                    this.sliceId(conversationId), false, this.recordCompletionHandler(worker)
                );
            return this.response(worker);
        } catch ({ message: error }) {
            return { error };
